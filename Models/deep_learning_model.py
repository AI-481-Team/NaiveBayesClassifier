...
class KerasModel:
    ...
    def fit(self, X, y):
        ...
        X_vectorized = self.cv.fit_transform(X).toarray()  # make it a matrix, the network can handle (not sparse matrix)

        #this will tell how the input shape of the network needs to look like
        #print(len(self.cv.get_feature_names()))
    ...
    def predict_proba(self, X):
        X_vectorized = self.cv.transform(X).toarray()
        probabilities = self.model.predict(X_vectorized)
